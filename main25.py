...
class MainGame():
    ...
    def createMyTank(self):
        ...
        #创建音乐对象，调用方法
        music=Music(r'Resource/music/start.wav')
        music.play()
    ...
    def handleContinuousMove(self):
        ...
        if pressed_any:
            ...
            if not hasattr(self, "_last_dir_log"):
                self._last_dir_log = None
                self._last_log_time = 0
            if p1.direction != self._last_dir_log or (now - self._last_log_time) > 300:
                txt = {'L': '左', 'R': '右', 'U': '上', 'D': '下'}[p1.direction]
                print(f'我方坦克向{txt}移动')
                self._last_dir_log = p1.direction
    ...
    def getEvents(self):
        ...
        for event in eventlist:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r and not MainGame.TANK_P1:
                    #调用创建我方坦克的方法
                    self.createMyTank()
                if MainGame.TANK_P1 and MainGame.TANK_P1.live:
                    if event.key == pygame.K_SPACE:
                        ...
                        if len(MainGame.Bullet_list) < 3:
                            ...
                            musicFrie=Music(r'Resource/music/fire.wav')
                            musicFrie.play()
                        else:
                            print('子弹数量不足')
...
class Tank(BaseItem):
    def __init__(self,left,top,speed=5):
        ...
        #新增属性：坦克的移动开关
        self.stop=True
        self.live=True#新增属性：用来记录坦克是否存活
...
class Bullet(BaseItem):
    ...
    def hitMyTank(self):
        ...
        if pygame.sprite.collide_rect(self,MainGame.TANK_P1):
            ...
            # EnemyTankShotMusic=Music(r'Resource/music/hit.wav')
            # EnemyTankShotMusic.play()
    def hitWalls (self):
        '''新增子弹与墙壁碰撞检测'''
        for wall in MainGame.Wall_list:
            if pygame.sprite.collide_rect(wall, self):
...
class Wall():
    def __init__(self,left,top):
        self.image=pygame.image.load(r'img/wall/steels.gif')
        self.rect=self.image.get_rect()
        self.rect.left=left
        self.rect.top=top
        #用来判断墙壁是否展示
